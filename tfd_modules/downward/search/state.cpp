...
bool Condition::is_module() const
{
...
{
	return os << condition.var<<"="<<condition.prev;
}

std::ostream & operator<<(std::ostream &os, const ScheduledEffect& eff)
{
	os << "at "<<eff.time_increment<<": "<<eff.var<<":="<<eff.pre<<"->"<<eff.post;
	os << " (s: ";
	for (size_t i = 0; i < eff.cond_start.size(); i++)
	{
		os << eff.cond_start[i] << " ";
	}
	os << ")(o: ";
	for (size_t i = 0; i < eff.cond_overall.size(); i++)
	{
		os << eff.cond_overall[i] << " ";
	}
	os << ")(e: ";
	for (size_t i = 0; i < eff.cond_end.size(); i++)
	{
		os << eff.cond_end[i] << " ";
	}
	return os <<")";
...
TimeStampedState::TimeStampedState(const TimeStampedState &predecessor, const Operator &op, int relaxed) :
		state(predecessor.state), scheduled_effects(predecessor.scheduled_effects), conds_over_all(
				predecessor.conds_over_all), conds_at_end(predecessor.conds_at_end), operators(predecessor.operators)
{
	...
	// satisfied.
	for (int i = 0; i < op.get_effects_end().size(); i++)
	{
		const Effect &eff = op.get_effects_end()[i];
	...
	for (int i = 0; i < op.get_mod_effs_end().size(); i++)
	{
		...
		{
			scheduled_module_effects.push_back(ScheduledModuleEffect(duration, mod_eff, &op));
		}
	}
	...
	for (int i = 0; i < op.get_effects_start().size(); i++)
	{
		const Effect &effect = op.get_effects_start()[i];

		// at-start effects may not have any at-end conditions
		assert(effect.cond_end.size() == 0);

		if (effect.does_fire(predecessor, &op, relaxed))
		{
			apply_effect(effect.var, effect.fop, effect.var_post, effect.post);
		}
	}
	...
	for (int i = 0; i < scheduled_module_effects.size(); i++)
	{
		...
		if ((eff.time_increment + g_parameters.epsTimeComparison < sep) && satisfies(eff.cond_end, eff.parent_op, relaxed))
		{
	...
	for (int i = 0; i < op.get_conditions_overall().size(); i++)
	{
		conds_over_all.push_back(ScheduledCondition(duration, op.get_conditions_overall()[i], &op));
	}
	...
	for (int i = 0; i < op.get_conditions_end().size(); i++)
	{
		conds_at_end.push_back(ScheduledCondition(duration, op.get_conditions_end()[i], &op));
	}
	//ROS_INFO_STREAM("scheduling operator: ");
	//op.dump();
...
TimeStampedState TimeStampedState::let_time_pass(bool go_to_intermediate_between_now_and_next_happening,
		bool skip_eps_steps, bool relaxed) const
{
	//ROS_INFO_STREAM("let time pass");
	...
	if (time_equals(nh, timestamp) || !go_to_intermediate_between_now_and_next_happening)
	{
		succ.timestamp = nh;
	}
	else
	...
	if (!go_to_intermediate_between_now_and_next_happening)
	{
		...
		for (int i = 0; i < scheduled_effects.size(); i++)
		{
			...
			{
				//ROS_INFO_STREAM("apply at end effect: "<<eff);
	...
	if (!go_to_intermediate_between_now_and_next_happening)
	{
		for (int i = 0; i < succ.scheduled_module_effects.size(); i++)
		{
			...
			{
				succ.scheduled_module_effects.erase(succ.scheduled_module_effects.begin() + i);
				i--;
			}
		}
	}
	...
	for (int i = 0; i < succ.conds_at_end.size(); i++)
	{
		...
		//ROS_INFO_STREAM(succ.conds_at_end[i]);
	}
	if (!go_to_intermediate_between_now_and_next_happening)
	{
		for (int i = 0; i < succ.conds_at_end.size(); i++)
...
void TimeStampedState::dump(bool verbose) const
{
	...
	if (verbose)
	{
		...
		for (int i = 0; i < scheduled_effects.size(); i++)
		{
			...
			{
				cout << g_variable_name[scheduled_effects[i].cond_end[j].var] << ": " << scheduled_effects[i].cond_end[j].prev;
			}
			cout << ">,<";
		...
		for (int i = 0; i < conds_over_all.size(); i++)
		{
			...
			cout << g_variable_name[conds_over_all[i].var] << ":" << conds_over_all[i].prev << ">>" << endl;
		}
		cout << " persistent at-end conditions:" << endl;
		for (int i = 0; i < conds_at_end.size(); i++)
		{
			cout << "  <" << (conds_at_end[i].time_increment + timestamp) << ",<";
			cout << g_variable_name[conds_at_end[i].var] << ":" << conds_at_end[i].prev << ">>" << endl;
		}
...
std::string TimeStampedState::toPDDL(bool logical, bool onlyTruePredicates, bool numerical, std::string separator,
		int maxLineLength) const
{
	...
	if (logical)
	{
		for (PredicateMapping::iterator it = g_pred_mapping.begin(); it != g_pred_mapping.end(); it++)
		{
	...
	if (numerical)
	{
		for (FunctionMapping::iterator it = g_func_mapping.begin(); it != g_func_mapping.end(); it++)
		{
			string pred = it->first;
			int var = it->second;
...
bool TimeStampedState::is_consistent_when_progressed(bool relaxed, TimedSymbolicStates* timedSymbolicStates) const
{
